package com.clothingstore.clothing_store_be.util;
...
public class VnpayUtil {
    ...
    public boolean verifySignature(Map<String, String> params) {
        ...
        if (receivedHash == null) {
            log.warn("VNPay verify failed: missing vnp_SecureHash, tmnCode={}, params={}", tmnCode, params);
            return false;
        }
        ...
        if (!computed.equalsIgnoreCase(receivedHash)) {
            log.warn(
                    "VNPay checksum mismatch: tmnCode={}, receivedHash={}, computedHash={}, queryString={}, filteredParams={}",
                    tmnCode,
                    receivedHash,
                    computed,
                    queryString,
            ...
            log.info("VNPay checksum verified: tmnCode={}, txnRef={}", tmnCode, params.get("vnp_TxnRef"));
        }
     ...
     *
     * @param txnRef          Mã tham chiếu giao dịch gốc (orderCode)
     * @param transactionNo   Mã giao dịch tại VNPay (vnp_TransactionNo)
     * @param amount          Số tiền hoàn (VND, chưa nhân 100)
     * @param transactionDate Ngày giao dịch gốc (vnp_PayDate) format yyyyMMddHHmmss
     * @param createdBy       Người tạo yêu cầu hoàn (email admin)
     * @param ipAddress       IP của admin
     * @param orderInfo       Thông tin hoàn tiền
     * @param transactionType 02 = hoàn toàn phần, 03 = hoàn một phần
     * @return Map chứa response từ VNPay
     */
    public Map<String, Object> refund(String txnRef, String transactionNo,
        ...
        // Checksum cho refund: hash theo pipe-delimited string
        ...
        log.info("VNPay refund request: url={}, body={}", apiUrl, jsonBody);
        ...
        try {
            ...
            // Parse simple JSON response to Map
            return parseJsonResponse(responseBody);
        } catch (Exception e) {
            log.error("VNPay refund error", e);
            throw new RuntimeException("Lỗi kết nối VNPay khi hoàn tiền: " + e.getMessage(), e);
        }
    }
